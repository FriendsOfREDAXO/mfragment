...
// SLIDE LOOP
if ($var['config']['slideshow']['uk-slideshow']['finite'] === 'true') {
    unset($var['config']['slideshow']['uk-slideshow']['finite']);
} else {
    $var['config']['slideshow']['uk-slideshow']['finite'] = 'true';
}
...
// VIEWPORT HEIGHT
$viewportHeight = '';
if (rex::isFrontend() && $var['config']['slideshow']['fullHeight'] === '1') {
    if (!empty($parallax)) {
        $var['config']['slideshow']['uk-height-viewport']['offset-bottom'] = '-' . ($var['config']['section']['parallax-offset'] / 2) . 'px';
    }
    $viewportHeight = ' uk-height-viewport="' . str_replace(['=', '%3A'], [':', ': '], http_build_query(array_filter($var['config']['slideshow']['uk-height-viewport']), null, '; ')) . '"';
    unset($var['config']['slideshow']['uk-slideshow']['ratio']);
}

// RENDER OUTPUTS
$slides = $this->getSubfragment("uikit/slide.items.php", $var);
$arrows = ($var['config']['slideshow']['showArrows'] === 'true') ? MFragment::parseUiKit('slidenav', []) : '';
$navigation = ($var['config']['slideshow']['showDotNavigation'] === 'true') ? MFragment::parseUiKit('dotnav', ['count' => count($var['slides'])]) : '';

$output = '
<section class="uk-section-' . $uniqueId . ' ' . implode(' ', $var['config']['section']['class']) . '"' . $sticky . '>
    <div class="' . implode(' ', $var['config']['container']['class']) . '">
        <div class="uk-position-relative uk-visible-toggle uk-light" uk-slideshow="' . str_replace(['=', '%3A'], [':', ': '], http_build_query(array_filter($var['config']['slideshow']['uk-slideshow']), null, '; ')) . '"' . $parallax . '>
            <' . (($var['config']['format'] == 'list') ? 'ul' : 'div') . ' class="uk-slideshow-items"' . $viewportHeight . '>
                ' . $slides . '
